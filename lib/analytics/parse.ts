...
/**
 * Undo Latin-1 misdecoding of a UTF-8 header value.
 *
 * HTTP headers are bytes. Cloudflare sends place names as UTF-8, but the header
 * is surfaced as Latin-1, so "Dasmariñas" (ñ = C3 B1) arrives as the two
 * characters "Ã±". Re-encoding to bytes and decoding as UTF-8 reverses it.
 *
 * The guards matter more than the conversion: a name that is *already* correct
 * must survive untouched. "Málaga" re-encoded to Latin-1 is not valid UTF-8, so
 * it fails decoding and is returned as-is; anything already outside the Latin-1
 * range is real Unicode and is left alone.
 */
function repairLatin1Mojibake(value: string): string {
  // Real multi-byte characters already present — nothing was misdecoded.
  if (/[^\u0000-\u00FF]/.test(value)) return value;
  // Pure ASCII cannot be mojibake.
  if (!/[\u0080-\u00FF]/.test(value)) return value;
  try {
    ...
    // Not valid UTF-8 when read as bytes, so it was correctly encoded already.
    return value;
  }
}
...
function cityFromHeaders(headers: Headers): string | null {
  ...
  city = repairLatin1Mojibake(city).trim();
...
export function geoFromHeaders(headers: Headers): Geo {
  const raw = firstHeader(headers, REGION_HEADERS);
  // Region comes from the same Latin-1 headers as city, so it needs the same
  // repair — "Región de Murcia" would otherwise store as "RegiÃ³n de Murcia".
  const region = raw ? repairLatin1Mojibake(raw) : null;
