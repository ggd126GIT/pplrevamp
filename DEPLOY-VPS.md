...
**Known gap:** the VPS `origin` remote points at a bare repo at `/home/gilbertd/ppl.git` pushed from
the workstation, because GitHub was not signed in when the deploy key was to be registered. To
finish §B1, add the VPS public key as a deploy key, then:
`cd /var/www/ppl && git remote set-url origin git@github.com:ggd126GIT/pplrevamp.git`
